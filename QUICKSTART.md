...
- **统计标签页**：实时监控会话、反馈、技能和规则的数量，以及 Coach 模块的运行状态。
- **技能标签页**：查看从成功经验中学到的技能。
- **规则标签页**：查看从失败经验中学到的规则。
- **Coach 标签页**：
    - **生成任务**：Coach Agent 根据业务目标生成有益的学习任务。
    - **运行任务**：Learner Agent 执行任务，Coach Agent 评估结果，并触发学习（Gym 模式）。
    - **任务列表**：查看所有 Coach 任务的状态和学习结果。
